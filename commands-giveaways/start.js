...
module.exports = class StartCommand extends Command {
    ...
    async run(client, message, args) {
        ...
        await msg.react('🎉')
        dbgive.prepare('INSERT INTO giveaways(guild_id,channel,message,creator,reward,amount,start_at,end_at) VALUES(?,?,?,?,?,?,?,?)').run(message.guild.id,channel.mentions.channels.first().id, msg.id, message.author.id, prize.content, parseInt(winners.content), Date.now(), Date.now() + ms(time.content));
